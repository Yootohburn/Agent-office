import { campaigns, CHANNEL_CONFIG, PIPELINE_STAGES, formatTHB } from '../../agents/campaignRegistry'
...
export default function OfficeSidebar({ selectedCampaignId, onSelectCampaign }: Props) {
  return (
            ...
            >
              <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', gap: 4, marginBottom: 4 }}>
                ...
                <span style={{ fontFamily: 'VT323, monospace', fontSize: 11, color: cfg.color, background: `${cfg.color}18`, padding: '0px 4px', flexShrink: 0, letterSpacing: 1 }}>
                  {cfg.label.slice(0, 1).toUpperCase() + cfg.label.slice(1, 2)}
                </span>
              </div>
              ...
              <div style={{ background: '#1a2540', height: 2, marginBottom: hasWarning ? 5 : 0 }}>
                <div style={{ background: cfg.color, height: '100%', width: `${c.progress}%` }} />
              </div>

              {hasWarning && (
                <div style={{ fontFamily: 'VT323, monospace', fontSize: 11, color: '#ff5252', marginTop: 3 }}>
